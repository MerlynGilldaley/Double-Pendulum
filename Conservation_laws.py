...
class Conservation(Double_Pendulum):
    ...
    def energy(self):
        ...
        self.kinetic_energy = 1/2 * self.mass1 * self.length1**2 * (2*self.omega1**2 + self.omega2**2 + 2*self.omega1*self.omega2*np.cos(self.theta1 - self.theta2))
        self.potential_energy = self.mass1 * self.g * self.length1 * (3 - 2*np.cos(self.theta1) * np.cos(self.theta2))
